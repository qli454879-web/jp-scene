...
3) insight_text: 中文 100~200字，一段自然的深度解析。像跟朋友聊天一样讲这个词怎么用。必须包含：这个词是外来语还是和语/汉语（如果是外来语，注明原词是什么；如果是和语/汉语则不用写原词）、日语中的意思、对应的中文意思、日常生活中的实际用法。只从以下角度挑1-2个补充（别全写，别列清单）：跟近义词的语感差异、背后反映的日本思维习惯、容易闹笑话的坑。绝对不要教读者这个词怎么读、怎么发音，不要分析声调或重音位置（发音/声调/重音由系统另外提供，不需要你写）。也不要出现「读成」「读错」「读法」「念成」「重音在」这类词。如果这个词的读法有特殊之处，忽略它。日常解析可以稍微提到这个词在游戏/动漫等特定领域的用法作为背景，但主体内容必须是日常生活中的意思和用法。不要写频率和例句（频率和例句已用独立字段输出）。不要写"该词""学习者""本词"这类翻译腔。不用加【】标签，直接写正文段落。用中文写，可以适当夹杂日语词汇，但不能通篇都是日文。
...
{{"具体场景A": 85, "具体场景B": 60, "具体场景C": 30}}
3) insight_text: 中文 150~300字，深度解析这个流行语。必须包含三部分：
【来源】20~40字，这个词是怎么来的（网络缩写/英语音译/和製英語/方言等），注明原词来源。
【原意】30~80字，该词原本在日语中的意思。
【流行语用法】80~200字，作为流行语的具体用法，包括在什么场景/人群中使用、表达什么情绪或态度、近一两年在日本社交网络上的使用趋势。禁止写任何发音/读音/声调/重音相关内容。不要写"该词""学习者""本词"这类翻译腔，像跟朋友科普流行梗一样自然地写。用中文写，可以适当夹杂日语词汇，但不能通篇都是日文。
...
【来源】20~40字，这个词的语言来源（英语音译/和製英語/缩写等），注明原词。
...
禁止写任何发音/读音/声调/重音相关内容。用中文写，可以适当夹杂日语词汇，但不能通篇都是日文。
